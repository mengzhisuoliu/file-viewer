...
![Flyfish Viewer 中文演示：Office、多格式文件与工具栏预览](docs/public/_media/flyfish-viewer-demo-zh.gif)
...
- 文本、代码和 Markdown 超过 `options.text.virtualizeAboveBytes`（默认 512 KiB）时会自动启用稀疏索引和有界虚拟行，避免把完整源码解码、语法高亮并挂载到 DOM。全文搜索仍覆盖完整源文件；超长单行按 `maxRenderedLineBytes`（默认 16 KiB）分段浏览，可用 `virtualOverscanLines` 调整可视区缓冲行数
