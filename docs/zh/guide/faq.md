...
`.epub` 由 `@file-viewer/renderer-epub` 独立承接。v2.2.2 的阅读引擎是包内固化、按需加载并附带 NOTICE 的本地 vendor 资产，不会从公网加载，也不会把旧版 XML 解析依赖带进业务生产依赖树。阅读区默认使用滚动文档模式，兼容性比超宽分页布局更稳，避免部分浏览器出现正文白板。当前不内置 Kindle 专有格式或 DRM 电子书解析，建议在业务侧转换为 EPUB / PDF。
