...
const ZH_CN_MESSAGES: Record<FileViewerMessageKey, string> = {
  ...
  'state.unsupported.install.description': '推荐安装 {presetPackage} 并启用 @file-viewer/vite-plugin（preset: \'{vitePreset}\' 或 preset: \'auto\'）{rendererTip}，也可以通过 options.renderers 手动传入对应 renderer。',
  ...
  'archive.warning.encrypted': '检测到加密内容，当前在线预览不接收密码，建议下载后本地解压。',
...
const EN_US_MESSAGES: Record<FileViewerMessageKey, string> = {
  ...
  'state.unsupported.install.description': 'Install {presetPackage} and enable @file-viewer/vite-plugin (preset: \'{vitePreset}\' or preset: \'auto\'){rendererTip}, or pass the matching renderer through options.renderers.',
  ...
  'archive.warning.encrypted': 'Encrypted content was detected. Online preview does not accept passwords; download and extract it locally.',
